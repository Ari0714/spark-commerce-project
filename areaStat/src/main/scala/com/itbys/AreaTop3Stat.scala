...
import com.itbys.commons.conf.ConfigurationManager
import com.itbys.commons.constant.Constants
import com.itbys.commons.utils.ParamUtils
...
object AreaTop3Stat {
  ...
  def getTop3Product(sparkSession: SparkSession, taskUUID: String): Unit = {
    ...
    top3ProductRDD.toDF().show(false)
...
//    val sql="select city_id,click_product_id from user_visit_action where date<='"+endDate+"' and date>='"+startDate+"' and click_product_id != -1"
    val sql="select city_id,click_product_id from user_visit_action where click_product_id != -1"
